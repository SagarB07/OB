 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE C_Project" +
      "        SET AD_Org_ID = (?) , Value = (?) , Name = (?) , IsSummary = (?) , Projectphase = (?) , Startdate = TO_DATE(?) , DateContract = TO_DATE(?) , DateFinish = TO_DATE(?) , Invoice_ToProject = (?) , SalesRep_ID = (?) , Responsible_ID = (?) , C_Location_ID = (?) , Description = (?) , PlannedAmt = TO_NUMBER(?) , PlannedQty = TO_NUMBER(?) , PlannedMarginAmt = TO_NUMBER(?) , IsCommitment = (?) , IsCommitCeiling = (?) , CommittedAmt = TO_NUMBER(?) , CommittedQty = TO_NUMBER(?) , Servrevenue = TO_NUMBER(?) , Expexpenses = TO_NUMBER(?) , Servcost = TO_NUMBER(?) , Expreinvoicing = TO_NUMBER(?) , Servmargin = TO_NUMBER(?) , Expmargin = TO_NUMBER(?) , C_BPartner_ID = (?) , C_BPartner_Location_ID = (?) , AD_User_ID = (?) , POReference = (?) , FIN_Paymentmethod_ID = (?) , C_PaymentTerm_ID = (?) , Createtemppricelist = (?) , M_PriceList_ID = (?) , C_Currency_ID = (?) , M_Warehouse_ID = (?) , ChangeProjectStatus = (?) , CopyFrom = (?) , GenerateOrder = (?) , Projectstatus = (?) , C_Project_ID = (?) , Processing = (?) , PaymentRule = (?) , GenerateTo = (?) , LastPlannedProposalDate = TO_DATE(?) , AccountNo = (?) , Document_Copies = TO_NUMBER(?) , Setprojecttype = (?) , M_PriceList_Version_ID = (?) , Projectkind = (?) , Note = (?) , AD_Client_ID = (?) , BillTo_ID = (?) , C_ProjectPhase_ID = (?) , PlannedPOAmt = TO_NUMBER(?) , IsActive = (?) , C_Campaign_ID = (?) , C_Phase_ID = (?) , C_ProjectType_ID = (?) , Processed = (?) , InvoicedQty = TO_NUMBER(?) , Publicprivate = (?) , InvoicedAmt = TO_NUMBER(?) , ProjectBalanceAmt = TO_NUMBER(?) , ProjectCategory = (?) , updated = now(), updatedby = ? " +
      "        WHERE C_Project.C_Project_ID = ? " +
      "        AND C_Project.AD_Client_ID IN (";
    strSql = strSql + ((adUserClient==null || adUserClient.equals(""))?"":adUserClient);
    strSql = strSql + 
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (AD_Org_ID, Value, Name, IsSummary, Projectphase, Startdate, DateContract, DateFinish, Invoice_ToProject, SalesRep_ID, Responsible_ID, C_Location_ID, Description, PlannedAmt, PlannedQty, PlannedMarginAmt, IsCommitment, IsCommitCeiling, CommittedAmt, CommittedQty, Servrevenue, Expexpenses, Servcost, Expreinvoicing, Servmargin, Expmargin, C_BPartner_ID, C_BPartner_Location_ID, AD_User_ID, POReference, FIN_Paymentmethod_ID, C_PaymentTerm_ID, Createtemppricelist, M_PriceList_ID, C_Currency_ID, M_Warehouse_ID, ChangeProjectStatus, CopyFrom, GenerateOrder, Projectstatus, C_Project_ID, Processing, GenerateTo, IsActive, LastPlannedProposalDate, AccountNo, Document_Copies, Setprojecttype, M_PriceList_Version_ID, Projectkind, Note, AD_Client_ID, BillTo_ID, C_ProjectPhase_ID, PlannedPOAmt, PaymentRule, C_Campaign_ID, C_Phase_ID, C_ProjectType_ID, Processed, InvoicedQty, Publicprivate, InvoicedAmt, ProjectBalanceAmt, ProjectCategory, created, createdby, updated, updatedBy)" +
