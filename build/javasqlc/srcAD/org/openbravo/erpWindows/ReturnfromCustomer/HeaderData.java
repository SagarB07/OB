 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE C_Order" +
      "        SET RM_ReceiveMaterials = (?) , DocAction = (?) , RM_PickFromShipment = (?) , RM_AddOrphanLine = (?) , RM_CreateInvoice = (?) , DocStatus = (?) , AD_Org_ID = (?) , GrandTotal = TO_NUMBER(?) , C_DocTypeTarget_ID = (?) , TotalLines = TO_NUMBER(?) , DocumentNo = (?) , C_Return_Reason_ID = (?) , C_Currency_ID = (?) , DateOrdered = TO_DATE(?) , C_BPartner_ID = (?) , C_BPartner_Location_ID = (?) , M_Warehouse_ID = (?) , InvoiceRule = (?) , C_PaymentTerm_ID = (?) , Description = (?) , FIN_Paymentmethod_ID = (?) , M_PriceList_ID = (?) , C_Project_ID = (?) , C_Activity_ID = (?) , C_Campaign_ID = (?) , AD_OrgTrx_ID = (?) , User1_ID = (?) , User2_ID = (?) , AD_User_ID = (?) , BillTo_ID = (?) , ChargeAmt = TO_NUMBER(?) , POReference = (?) , C_Charge_ID = (?) , SalesRep_ID = (?) , DateAcct = TO_DATE(?) , DatePromised = TO_DATE(?) , Processed = (?) , DatePrinted = TO_DATE(?) , DeliveryRule = (?) , FreightCostRule = (?) , IsSOTrx = (?) , PaymentRule = (?) , IsDiscountPrinted = (?) , Posted = (?) , IsTaxIncluded = (?) , IsSelected = (?) , DropShip_User_ID = (?) , DropShip_BPartner_ID = (?) , CopyFrom = (?) , DropShip_Location_ID = (?) , IsSelfService = (?) , Generatetemplate = (?) , Delivery_Location_ID = (?) , Deliverynotes = (?) , C_Incoterms_ID = (?) , Incotermsdescription = (?) , CopyFromPO = (?) , IsPrinted = (?) , IsInvoiced = (?) , IsDelivered = (?) , C_DocType_ID = (?) , PriorityRule = (?) , IsActive = (?) , AD_Client_ID = (?) , M_Shipper_ID = (?) , C_Order_ID = (?) , DeliveryViaRule = (?) , FreightAmt = TO_NUMBER(?) , Processing = (?) , updated = now(), updatedby = ? " +
      "        WHERE C_Order.C_Order_ID = ? " +
      "        AND C_Order.AD_Client_ID IN (";
    strSql = strSql + ((adUserClient==null || adUserClient.equals(""))?"":adUserClient);
    strSql = strSql + 
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        INSERT INTO C_Order " +
      "        (RM_ReceiveMaterials, DocAction, RM_PickFromShipment, RM_AddOrphanLine, RM_CreateInvoice, DocStatus, AD_Org_ID, GrandTotal, C_DocTypeTarget_ID, TotalLines, DocumentNo, C_Return_Reason_ID, C_Currency_ID, DateOrdered, C_BPartner_ID, C_BPartner_Location_ID, M_Warehouse_ID, InvoiceRule, C_PaymentTerm_ID, Description, FIN_Paymentmethod_ID, M_PriceList_ID, C_Project_ID, C_Activity_ID, C_Campaign_ID, AD_OrgTrx_ID, User1_ID, User2_ID, AD_User_ID, BillTo_ID, ChargeAmt, POReference, C_Charge_ID, SalesRep_ID, DateAcct, DatePromised, Processed, DatePrinted, DeliveryRule, FreightCostRule, IsSOTrx, PaymentRule, IsDiscountPrinted, Posted, IsTaxIncluded, IsSelected, DropShip_User_ID, DropShip_BPartner_ID, CopyFrom, DropShip_Location_ID, IsSelfService, Generatetemplate, Delivery_Location_ID, Deliverynotes, C_Incoterms_ID, Incotermsdescription, CopyFromPO, IsPrinted, IsInvoiced, IsDelivered, C_DocType_ID, PriorityRule, IsActive, AD_Client_ID, M_Shipper_ID, C_Order_ID, DeliveryViaRule, FreightAmt, Processing, created, createdby, updated, updatedBy)" +
