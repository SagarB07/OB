 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        SET AD_Org_ID = (?) , Line = TO_NUMBER(?) , C_ValidCombination_ID = (?) , Description = (?) , C_Currency_ID = (?) , AmtSourceDr = TO_NUMBER(?) , AmtSourceCr = TO_NUMBER(?) , IsGenerated = (?) , AmtAcctDr = TO_NUMBER(?) , AmtAcctCr = TO_NUMBER(?) , CurrencyRateType = (?) , CurrencyRate = TO_NUMBER(?) , Open_Items = (?) , FIN_Financial_Account_ID = (?) , FIN_Paymentmethod_ID = (?) , C_Glitem_ID = (?) , Paymentdate = TO_DATE(?) , FIN_Payment_ID = (?) , C_UOM_ID = (?) , Qty = TO_NUMBER(?) , C_Debt_Payment_ID = (?) , C_Withholding_ID = (?) , IsActive = (?) , C_Tax_ID = (?) , EM_Aprm_Addpayment = (?) , C_Bpartner_ID = (?) , M_Product_ID = (?) , C_Project_ID = (?) , C_Activity_ID = (?) , C_Campaign_ID = (?) , C_Salesregion_ID = (?) , User1_ID = (?) , User2_ID = (?) , A_Asset_ID = (?) , C_Costcenter_ID = (?) , AD_Client_ID = (?) , GL_Journal_ID = (?) , DateAcct = TO_DATE(?) , GL_JournalLine_ID = (?) , updated = now(), updatedby = ? " +
      "        WHERE GL_JournalLine.GL_JournalLine_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (AD_Org_ID, Line, C_ValidCombination_ID, Description, C_Currency_ID, AmtSourceDr, AmtSourceCr, IsGenerated, AmtAcctDr, AmtAcctCr, CurrencyRateType, CurrencyRate, Open_Items, FIN_Financial_Account_ID, FIN_Paymentmethod_ID, C_Glitem_ID, Paymentdate, FIN_Payment_ID, C_UOM_ID, Qty, C_Debt_Payment_ID, C_Withholding_ID, IsActive, C_Tax_ID, EM_Aprm_Addpayment, C_Bpartner_ID, M_Product_ID, C_Project_ID, C_Activity_ID, C_Campaign_ID, C_Salesregion_ID, User1_ID, User2_ID, A_Asset_ID, C_Costcenter_ID, AD_Client_ID, GL_Journal_ID, DateAcct, GL_JournalLine_ID, created, createdby, updated, updatedBy)" +
