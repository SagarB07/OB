 ...
 */
  ...
  public int update(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      "        UPDATE C_OrderLine" +
      "        SET Line = TO_NUMBER(?) , M_Product_ID = (?) , M_Inoutline_ID = (?) , EM_Re_Partidaproducto = (?) , QtyOrdered = TO_NUMBER(?) , M_AttributeSetInstance_ID = (?) , EM_RE_Value_Producto = (?) , C_Return_Reason_ID = (?) , C_UOM_ID = (?) , PriceActual = TO_NUMBER(?) , Gross_Unit_Price = TO_NUMBER(?) , LineNetAmt = TO_NUMBER(?) , Line_Gross_Amount = TO_NUMBER(?) , C_Tax_ID = (?) , PriceList = TO_NUMBER(?) , GrossPriceList = TO_NUMBER(?) , Discount = TO_NUMBER(?) , M_Warehouse_Rule_ID = (?) , Description = (?) , Create_Reservation = (?) , Taxbaseamt = TO_NUMBER(?) , QtyInvoiced = TO_NUMBER(?) , QtyDelivered = TO_NUMBER(?) , DateOrdered = TO_DATE(?) , DatePromised = TO_DATE(?) , M_Warehouse_ID = (?) , QtyReserved = TO_NUMBER(?) , M_Shipper_ID = (?) , C_BPartner_ID = (?) , DirectShip = (?) , FreightAmt = TO_NUMBER(?) , C_BPartner_Location_ID = (?) , CANCELPRICEAD = (?) , M_Product_Uom_Id = (?) , QuantityOrder = TO_NUMBER(?) , grosspricestd = TO_NUMBER(?) , PriceStd = TO_NUMBER(?) , Iseditlinenetamt = (?) , Quotationline_ID = (?) , SO_Res_Status = (?) , Manage_Reservation = (?) , AD_Org_ID = (?) , C_Project_ID = (?) , C_Costcenter_ID = (?) , A_Asset_ID = (?) , User1_ID = (?) , User2_ID = (?) , Explode = (?) , BOM_Parent_ID = (?) , AD_Client_ID = (?) , S_ResourceAssignment_ID = (?) , IsDescription = (?) , M_Offer_ID = (?) , PriceLimit = TO_NUMBER(?) , C_Order_Discount_ID = (?) , C_OrderLine_ID = (?) , ChargeAmt = TO_NUMBER(?) , C_Charge_ID = (?) , Manage_Prereservation = (?) , C_Currency_ID = (?) , DateInvoiced = TO_DATE(?) , DateDelivered = TO_DATE(?) , C_Order_ID = (?) , IsActive = (?) , Ref_OrderLine_ID = (?) , updated = now(), updatedby = ? " +
      "        WHERE C_OrderLine.C_OrderLine_ID = ? " +
  ...
  public int insert(Connection conn, ConnectionProvider connectionProvider)    throws ServletException {
    ...
    strSql = strSql + 
      ...
      "        (Line, M_Product_ID, M_Inoutline_ID, EM_Re_Partidaproducto, QtyOrdered, M_AttributeSetInstance_ID, EM_RE_Value_Producto, C_Return_Reason_ID, C_UOM_ID, PriceActual, Gross_Unit_Price, LineNetAmt, Line_Gross_Amount, C_Tax_ID, PriceList, GrossPriceList, Discount, M_Warehouse_Rule_ID, Description, Create_Reservation, Taxbaseamt, QtyInvoiced, QtyDelivered, DateOrdered, DatePromised, M_Warehouse_ID, QtyReserved, M_Shipper_ID, C_BPartner_ID, DirectShip, FreightAmt, C_BPartner_Location_ID, CANCELPRICEAD, M_Product_Uom_Id, QuantityOrder, grosspricestd, PriceStd, Iseditlinenetamt, Quotationline_ID, SO_Res_Status, Manage_Reservation, AD_Org_ID, C_Project_ID, C_Costcenter_ID, A_Asset_ID, User1_ID, User2_ID, Explode, BOM_Parent_ID, AD_Client_ID, S_ResourceAssignment_ID, IsDescription, M_Offer_ID, PriceLimit, C_Order_Discount_ID, C_OrderLine_ID, ChargeAmt, C_Charge_ID, Manage_Prereservation, C_Currency_ID, DateInvoiced, DateDelivered, C_Order_ID, IsActive, Ref_OrderLine_ID, created, createdby, updated, updatedBy)" +
