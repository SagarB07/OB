-- Function: no_procesa_pago_quincena(character varying)

-- DROP FUNCTION no_procesa_pago_quincena(character varying);

CREATE OR REPLACE FUNCTION no_procesa_pago_quincena(lineaquincena character varying)
  RETURNS void AS
$BODY$ DECLARE 
...
BEGIN
 ...
 ---UPDATE IDT_CONTRATO SET i_errormsg = 'XXXXXXXXX';
v_c_glitem_id = (select c_glitem_id from c_glitem where name ilike '%Sueldo por pagar%');
select ad_table_id
        ...
        FOR Cur_AvanceSchema IN
         (select rql.no_registra_quinc_line_id,
            ...
            where rq.no_registra_quincena_id = rql.no_registra_quincena_id
             ...
             and rql.no_registra_quinc_line_id =lineaQuincena)
        ...
        LOOP
            ...
            v_cuenta_por_pagar = (SELECT em_no_pago_acct FROM C_BPARTNER WHERE C_BPARTNER_ID = v_bpartner_id);
			v_User_ID:=Cur_AvanceSchema.createdby;
			v_Org_ID:=Cur_AvanceSchema.AD_Org_ID;
			v_Client_ID:=Cur_AvanceSchema.AD_Client_ID;

	   update idt_contrato  set i_errormsg = v_fin_financial_account_id;
        ...
        --update no_registra_quinc_line
          -- set processed = 'Y'
         --where no_registra_quinc_line_id = lineaQuincena;
...
END ; $BODY$
  ...
  COST 100;
ALTER FUNCTION no_procesa_pago_quincena(character varying)
  OWNER TO tad;
