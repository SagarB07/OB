...
BEGIN
	...
	-- SELECT * FROM no_pago_cabecera WHERE no_pago_cabecera_id = '455F9AE2BE26473896EE54A9FC78B17C'
	-- SELECT * FROM c_doctype WHERE c_doctype_ID = '042C0D9E0BEF42E7B5F2B6E122031D38'
        --select t.tablename from ad_table t where t.ad_table_id = 
       -- UPDATE  IDT_CONTRATO SET I_ERRORMSG = V_TIPODOC || '-----' || V_PERIODO ;
	-- SELECT * FROM c_doctype WHERE c_doctype_id = '77C98C7F3FAC4D5F89A05A2232EB16D9';
	-- SELECT * FROM 
	-- SELECT * FROM no_rol_pago_provision
	-- SELECT  * FROM no_registra_quincena
        -- SELECT I_ERRORMSG FROM IDT_CONTRATO
        -- select * from no_registra_quinc_line quindet where quindet.c_doctype_id = 'E7EC0B4879C04A9EA2807C3209E7363B'
        -- ANTICIPO DE SUELDO
        -- SELECT * FROM c_doctype WHERE NAME LIKE '%Registro de quincenas%'
        -- SELECT * FROM 
        IF exists (select* from no_rol_pago_provision rocab where rocab.c_doctype_id = V_TIPODOC AND ISPAGO = 'Y' ) THEN 
      --  UPDATE  IDT_CONTRATO SET I_ERRORMSG = 'ROL DE PAGO';
      --  SELECT * FROM C_DOCTYPE WHERE NAME ILIKE '%ROl de Provisiones%';
	...
	-- CODIGO QUEMADO QUINCENAS
        IF V_TIPODOC = '77C98C7F3FAC4D5F89A05A2232EB16D9' THEN 
