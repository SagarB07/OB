...
vars.getGlobalVariable("inpglJournalId", windowId + "|GL_Journal_ID");
  ...
  private void refreshParentSession(VariablesSecureApp vars, String strPGL_Journal_ID) throws IOException,ServletException {
      ...
      if (data==null || data.length==0) return;
          vars.setSessionValue(windowId + "|AD_Org_ID", data[0].adOrgId);    vars.setSessionValue(windowId + "|C_AcctSchema_ID", data[0].cAcctschemaId);    vars.setSessionValue(windowId + "|C_DocType_ID", data[0].cDoctypeId);    vars.setSessionValue(windowId + "|DateDoc", data[0].datedoc);    vars.setSessionValue(windowId + "|DateAcct", data[0].dateacct);    vars.setSessionValue(windowId + "|C_Period_ID", data[0].cPeriodId);    vars.setSessionValue(windowId + "|C_Currency_ID", data[0].cCurrencyId);    vars.setSessionValue(windowId + "|CurrencyRateType", data[0].currencyratetype);    vars.setSessionValue(windowId + "|CurrencyRate", data[0].currencyrate);    vars.setSessionValue(windowId + "|Posted", data[0].posted);    vars.setSessionValue(windowId + "|GL_Category_ID", data[0].glCategoryId);    vars.setSessionValue(windowId + "|Processed", data[0].processed);    vars.setSessionValue(windowId + "|GL_Journal_ID", data[0].glJournalId);    vars.setSessionValue(windowId + "|AD_Client_ID", data[0].adClientId);    vars.setSessionValue(windowId + "|GL_JournalBatch_ID", data[0].glJournalbatchId);
