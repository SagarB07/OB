...
public class SL_Order_Product_Partida extends HttpSecureAppServlet {
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strUOM,
    ...
    // Completar datos
    String strvalue = SLOrderProductPartidaData.selectvalue(this, strMProductID);
    String strpartidaarancelaria = SLOrderProductPartidaData.selectprodcutoarancelaria(this,
        strMProductID);
    // Completar datos

    if (!strMProductID.equals("")) {
      SLOrderProductPartidaData[] dataOrder = SLOrderProductPartidaData.select(this, strCOrderId);

      if (log4j.isDebugEnabled())
        log4j.debug("get Offers date: " + dataOrder[0].dateordered + " partner:"
            + dataOrder[0].cBpartnerId + " prod:" + strMProductID + " std:"
            + strPriceStd.replace("\"", ""));
      strPriceActual = SLOrderProductPartidaData.getOffersPrice(this, dataOrder[0].dateordered,
    ...
    resultado.append("var calloutName='SL_Order_Product_Partida';\n\n");
    resultado.append("var respuesta = new Array(");

    // Datos Completos
    resultado.append("new Array(\"inpemReValueProducto\", \"" + strvalue + "\"),");
    resultado.append("new Array(\"inpemRePartidaproducto\", \"" + strpartidaarancelaria + "\"),");
    ...
    if (!strMProductID.equals("")) {
      ...
      strHasSecondaryUOM = SLOrderProductPartidaData.hasSecondaryUOM(this, strMProductID);
      resultado.append("new Array(\"inphasseconduom\", " + strHasSecondaryUOM + "),\n");
    }

    String strCTaxID = "";
    String orgLocationID = SLOrderProductPartidaData.getOrgLocationId(this,
    ...
    // SLOrderProductPartidaData.strMProductUOMID(this,strMProductID,strUOM);
