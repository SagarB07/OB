...
public class SE_Order_BPartner_Validacion extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException,
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      String strDoctypetargetId = vars.getStringParameter("inpcDoctypetargetId");
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strBPartner,
      ...
      String strPriceList0, String strDeliveryViaRule0, String strOrgId, String strDoctypetargetId)
    ...
    if (data != null && data.length > 0) {
      ...
      strUserRep = SEOrderBPartnerValidacionData.userIdSalesRep(this, data[0].salesrepId);
      ...
      if (strPriceList.equalsIgnoreCase("")) {
        strPriceList = SEOrderBPartnerValidacionData.defaultPriceList(this, strIsSOTrx,
            vars.getClient());
      }
    ...
    strMwarehouse = SEOrderBPartnerValidacionData.mWarehouse(this, strBPartner);
    ...
    resultado.append("new Array(\"inpisdiscountprinted\", \""
        + SEOrderBPartnerValidacionData.getIsDicountPrinted(this, strBPartner) + "\"),");
    ...
    // Pagos pendientes
    String strnuevo = "0";//SEOrderBPartnerValidacionData.pagos(this, strBPartner);

    // Documento de retorno
    String strRetorno = "N";//SEValidacionDocumentoData.esRetorno(this, strDoctypetargetId);

    // Aumento de validacion de pagos pendientes para el retorno del mensaje
    if ((data != null && data.length > 0
        && new BigDecimal(data[0].creditavailable).compareTo(BigDecimal.ZERO) < 0 && strIsSOTrx
          .equals("Y")) || (Integer.parseInt(strnuevo) != 0 && strRetorno.equals("N"))) {
      // Verifica si existen pagos vencidos
      if (Integer.parseInt(strnuevo) != 0) {
        // Envia el mensaje
        if (message.length() > 0)
          message.append("<br>");
        message.append("Tiene " + strnuevo
            + Utility.messageBD(this, "RE_PagosPendientes", vars.getLanguage()));
      }
      ...
      // Validacion para no permitir PosOrder con credito de limite insuficiente

      FieldProvider[] tipodoc = null;
      try {
        ComboTableData comboTableData = new ComboTableData(vars, this, "TABLEDIR", "C_Doctype_Id",
            "", "Tipo documento SO", Utility.getContext(this, vars, "#AccessibleOrgTree",
                strWindowId), Utility.getContext(this, vars, "#User_Client", strWindowId), 0);
        Utility.fillSQLParameters(this, vars, null, comboTableData, strWindowId, "");
        tipodoc = comboTableData.select(false);
        comboTableData = null;
      } catch (Exception ex) {
        throw new ServletException(ex);
      }

      resultado.append(",new Array(\"inpcDoctypetargetId\", ");
      if (tipodoc != null && tipodoc.length > 0) {
        resultado.append("new Array(");
        for (int i = 0; i < tipodoc.length; i++) {
          resultado.append("new Array(\"" + tipodoc[i].getField("id") + "\", \""
              + FormatUtilities.replaceJS(tipodoc[i].getField("name")) + "\", \"" + ("false")
              + "\")");
          if (i < tipodoc.length - 1)
            resultado.append(",\n");
        }
        resultado.append("\n)");
      } else
      ...
      FieldProvider[] tipodoctodo = null;
      try {
        ComboTableData comboTableData = new ComboTableData(vars, this, "TABLEDIR", "C_Doctype_Id",
            "", "C_DocType PO/SO",
        ...
        Utility.fillSQLParameters(this, vars, null, comboTableData, strWindowId, "");
        tipodoctodo = comboTableData.select(false);
        comboTableData = null;
      } catch (Exception ex) {
        throw new ServletException(ex);
      }
      ...
      resultado.append(",new Array(\"inpcDoctypetargetId\", ");
      if (tipodoctodo != null && tipodoctodo.length > 0) {
        resultado.append("new Array(");
        for (int i = 0; i < tipodoctodo.length; i++) {
          resultado.append("new Array(\"" + tipodoctodo[i].getField("id") + "\", \""
              + FormatUtilities.replaceJS(tipodoctodo[i].getField("name")) + "\", \"" + ("false")
              + "\")");
          if (i < tipodoctodo.length - 1)
            resultado.append(",\n");
        }
        resultado.append("\n)");
      } else
