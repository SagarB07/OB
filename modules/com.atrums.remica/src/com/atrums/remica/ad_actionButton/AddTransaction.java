package com.atrums.remica.ad_actionButton;
...
public class AddTransaction extends HttpSecureAppServlet {
  ...
  /*
   * private void excecuteSP(ConnectionProvider conn, String status, String paymentId) {
   * PreparedStatement st = null;
   * 
   * try { st = conn.getPreparedStatement("select re_payment_status('" + status + "', '" + paymentId
   * + "')"); st.execute();
   * 
   * } catch (Exception e) { // TODO Auto-generated catch block e.printStackTrace(); } finally { try
   * { conn.releasePreparedStatement(st); } catch (Exception ignore) { ignore.printStackTrace(); } }
   * }
   */
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strOrgId,
    ...
    XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
        "com/atrums/remica/ad_actionButton/AddTransaction").createXmlDocument();
  ...
  private void printGrid(HttpServletResponse response, VariablesSecureApp vars,
    ...
    XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
        "com/atrums/remica/ad_actionButton/AddTransactionGrid").createXmlDocument();

    OBContext.setAdminMode();
    try {
      // From AddPaymentFromTransaction the payment has been deposited and
