...
public class CO_Invoice_BPartner_SRI extends HttpSecureAppServlet
{
	...
	if (vars.commandIn("DEFAULT"))
	{
	    ...
	    String strfinPaymentmethodId = vars
		    .getStringParameter("inpfinPaymentmethodId");
	    String strOrgId = vars.getStringParameter("inpadOrgId");

	    try
	    {
		if ("inpfinPaymentmethodId".equals(strChanged))
		{ // Payment Method changed
		    printPagePaymentMethod(response, vars, strBPartner,
					   strIsSOTrx, strfinPaymentmethodId,
					   strOrgId);
		}
		else
		{
		    printPage(response, vars, strBPartner, strDocType,
			      strIsSOTrx, strWindowId, strLocation, strContact,
			      strProjectId, strTabId, strOrgId);
		}
	    }
    ...
    private void printPage(HttpServletResponse response,
	    VariablesSecureApp vars, String strBPartner, String strDocType,
	    String strIsSOTrx, String strWindowId, String strLocation,
	    String strContact, String strProjectId, String strTabId,
	...
	String strUserRep;
	String DocBaseType = COInvoiceBPartnerData
		.docBaseType(this, strDocType);
	StringBuffer resultado = new StringBuffer();
	resultado.append("var calloutName='CO_Invoice_BPartner_SRI';\n\n");
	if (data == null || data.length == 0)
	    resultado
		    .append("var respuesta = new Array(new Array(\"inpcBpartnerLocationId\", null));");

	else
	{
	    resultado.append("var respuesta = new Array(");
	    strUserRep = COOrderBPartnerData.userIdSalesRep(this,
							    data[0].salesrepId);
	    ...
	    {
		strPriceList = COOrderBPartnerData
			.defaultPriceList(this, strIsSOTrx, vars.getClient());
	    }
	    resultado.append("new Array(\"inpmPricelistId\", \""
		    + (strPriceList.equals("") ? Utility
			    .getContext(this, vars, "#M_PriceList_ID",
		...
		if (strPaymentRule.equals("S") || strPaymentRule.equals("U")
			&& strIsSOTrx.equals("Y"))
		    strPaymentRule = "P";
	    ...
	    {
		BpartnerMiscData[] paymentTerm = BpartnerMiscData
			.selectPaymentTerm(this, strOrgId, vars.getClient());
		...
		ComboTableData comboTableData = new ComboTableData(vars, this,
			"TABLEDIR", "C_BPartner_Location_ID", "",
		...
		for (int i = 0; i < tdv.length; i++)
		{
		    resultado
			    .append("new Array(\""
				    ...
				    + "\", \""
				    + (tdv[i].getField("id")
					    .equalsIgnoreCase(strLocation) ? "true"
					    : "false") + "\")");
		    if (i < tdv.length - 1)
			resultado.append(",\n");
		}
		...
		for (int i = 0; i < tld.length; i++)
		{
		    resultado
			    .append("new Array(\""
				    ...
				    + "\", \""
				    + (tld[i].getField("id")
					    .equalsIgnoreCase(strUserRep) ? "true"
					    : "false") + "\")");
		    if (i < tld.length - 1)
			resultado.append(",\n");
		}
		...
		for (int i = 0; i < tdv.length; i++)
		{
		    resultado
			    .append("new Array(\""
				    ...
				    + "\", \""
				    + (tdv[i].getField("id")
					    .equalsIgnoreCase(strContact) ? "true"
					    : "false") + "\")");
		    if (i < tdv.length - 1)
			resultado.append(",\n");
		}
		...
		resultado.append("null");
	    resultado.append("\n),");
	    String strWithHolding = COInvoiceBPartnerData
		    .WithHolding(this, strBPartner);
		...
		resultado.append(", new Array('MESSAGE', \""
			+ Utility.messageBD(this, "CreditLimitOver",
					    vars.getLanguage())
	    ...
	    {
		COInformacionSRIData[] dataSRI = COInformacionSRIData
			.selectBP(this, strBPartner);

		resultado.append(",new Array(\"inpemCoNroEstab\", \""
			+ dataSRI[0].estab + "\"),");
		resultado.append("new Array(\"inpemCoPuntoEmision\", \""
			+ dataSRI[0].emision + "\"),");
		resultado.append("new Array(\"inpemCoNroAutSri\", \""
			+ dataSRI[0].noautoriza + "\"),");
		resultado.append("new Array(\"inpemCoVencimientoAutSri\", \""
			+ dataSRI[0].fecautoriza + "\")");
	    }
    ...
    private void printPagePaymentMethod(HttpServletResponse response,
	    ...
	    String strfinPaymentmethodId, String strOrgId) throws IOException,
	    ServletException
    {
	XmlDocument xmlDocument = xmlEngine
		.readXmlTemplate("org/openbravo/erpCommon/ad_callouts/CallOut")
	...
	String message = isAutomaticCombination(vars, strBPartnerId,
						strIsSOTrx,
						strfinPaymentmethodId, strOrgId);

	result.append("var calloutName='CO_Invoice_BPartner';\n\n");
     ...
     */
    private String isAutomaticCombination(VariablesSecureApp vars,
	    String strBPartnerId, String strIsSOTrx,
	    String strfinPaymentmethodId, String strOrgId)
    {
	...
	try
	{
	    ...
	    if (bpartner != null && selectedPaymentMethod != null
		    && !"".equals(strOrgId))
	    {
		account = (isSales) ? bpartner.getAccount() : bpartner
		...
		if (account != null)
		{
		    ...
		    obc.add(Restrictions
			    .eq(FinAccPaymentMethod.PROPERTY_ACCOUNT, account));
		    obc.add(Restrictions
			    .eq(FinAccPaymentMethod.PROPERTY_PAYMENTMETHOD,
				selectedPaymentMethod));
		    obc.add(Restrictions
			    .in(FinAccPaymentMethod.PROPERTY_ORGANIZATION
				    + ".id", OBContext.getOBContext()
