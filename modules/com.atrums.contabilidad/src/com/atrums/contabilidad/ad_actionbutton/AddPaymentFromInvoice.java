...
public class AddPaymentFromInvoice extends HttpSecureAppServlet {
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars,
    ...
    XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
        "com/atrums/contabilidad/ad_actionbutton/AddPaymentFromInvoice").createXmlDocument();
    ...
    if ((financialAccounts.get(0).getCoNumeroChequeDesde() != null)
        && (financialAccounts.get(0).getCoNumeroChequeHasta() != null)) {

      Integer intDesde = Integer.valueOf(financialAccounts.get(0).getCoNumeroChequeDesde()
          .toString());
      Integer intHasta = Integer.valueOf(financialAccounts.get(0).getCoNumeroChequeHasta()
          .toString());

      if (financialAccounts.get(0).getCoNumeroChequeAhora() != null) {

        Integer intActual = Integer.valueOf(financialAccounts.get(0).getCoNumeroChequeAhora()
            .toString());

        if (intActual < intDesde || intActual > intHasta) {
          xmlDocument.setParameter("strnumerocheque", "Número de cheque esta fuera del rango");
        } else {
          xmlDocument.setParameter("strnumerocheque", intActual.toString());
        }
      } else {
        xmlDocument.setParameter("strnumerocheque", intDesde.toString());
      }
    }
  ...
  private void refreshFinancialAccountCombo(HttpServletResponse response, VariablesSecureApp vars,
    ...
    try {
      ...
      if ((financialAccount.getCoNumeroChequeDesde() != null)
          && (financialAccount.getCoNumeroChequeHasta() != null)) {

        Integer intDesde = Integer.valueOf(financialAccount.getCoNumeroChequeDesde().toString());
        Integer intHasta = Integer.valueOf(financialAccount.getCoNumeroChequeHasta().toString());

        if (financialAccount.getCoNumeroChequeAhora() != null) {

          Integer intActual = Integer.valueOf(financialAccount.getCoNumeroChequeAhora()
              .toString());

          if (intActual < intDesde || intActual > intHasta) {
            msg.put("strmensaje", "El número de cheque esta fuera de rango");
          } else {
            msg.put("strnumerocheque", intActual.toString());
          }
        } else {
          msg.put("strnumerocheque", intDesde.toString());
        }
      } else {
        msg.put("strmensaje", "");
      }
