...
package com.atrums.conciliacionec.dao;
...
public class TransactionsReconcileDao {
...
public static FieldProvider[] getTransactionsFiltered(FIN_FinancialAccount account,
    ...
    try {
      ...
      final List<Object> parameters = new ArrayList<Object>();
      final StringBuilder whereClause = new StringBuilder();
      whereClause.append(" as atce");
      whereClause.append(" left outer join atce.reconciliation as rec");
      whereClause.append(" where atce.financialAccount.id = ?");
      whereClause.append(" and (rec is null or rec.processed = 'N')");
      whereClause.append(" and atce.processed = 'Y'");
     
      parameters.add(account.getId());
      
      if (hideAfterDate) {
        whereClause.append(" and atce.transactionDate < ?");
        parameters.add(statementDate);
      }
      whereClause.append(" order by atce.transactionDate");

      final OBQuery<atecdpFinaccTransactionV> obquery1 = OBDal.getInstance().createQuery(
          atecdpFinaccTransactionV.class, whereClause.toString(), parameters);
      
      List<atecdpFinaccTransactionV> depositosOBList = obquery1.list();
      
      atecdpFinaccTransactionV[] ATEC_Depositos = new atecdpFinaccTransactionV[0];   
      ATEC_Depositos = depositosOBList.toArray(ATEC_Depositos);   
      FieldProvider[] data = FieldProviderFactory.getFieldProviderArray(depositosOBList);
      
      for (int i = 0; i < data.length; i++) {
        String strPaymentDocNo = "";
        String strBusinessPartner ="";
       
        FieldProviderFactory.setField(data[i], "transactionId", ATEC_Depositos[i].getId());
        FieldProviderFactory.setField(data[i], "transactionDate",
            dateFormater.format(ATEC_Depositos[i].getTransactionDate()));
       
        
         if (ATEC_Depositos[i].getFinancialAccount().getFINPaymentList() != null) {
        	 
        	 strPaymentDocNo = ATEC_Depositos[i].getDeposito();
        	 
        	 if(!"".equals(strPaymentDocNo)) {
        		 
	        	 final Query partnerQry = OBDal.getInstance().getSession().createQuery("select distinct(pay.businessPartner.name) as bpartner from FIN_Payment as pay where (pay.atecdpDeposito =:depositono or pay.documentNo=:depositono)");
	        	 partnerQry.setParameter("depositono", strPaymentDocNo);
	        	 List<Object> qryList = partnerQry.list();
	        	 
	        	 for(Iterator itr=qryList.listIterator();itr.hasNext();) {        		 
	        		   		       		 
	        		 
	        		if(strBusinessPartner.equals(""))
	        			 strBusinessPartner =itr.next().toString();
	        		 else
	        			 strBusinessPartner+=",".concat(itr.next().toString());
		...
		if(strBusinessPartner.length() > 30)
			FieldProviderFactory.setField(data[i], "businessPartnerTrunc", strBusinessPartner.substring(0, 27).concat("...").toString());
		else
			FieldProviderFactory.setField(data[i], "businessPartner",strBusinessPartner);
            
        FieldProviderFactory.setField(data[i], "deposito", strPaymentDocNo);

        // Truncate description
        String description = ATEC_Depositos[i].getDescription();
        ...
        FieldProviderFactory.setField(data[i], "valorPagado", ATEC_Depositos[i].getValorPagado()
            .toString());
        FieldProviderFactory.setField(data[i], "valorDepositado", ATEC_Depositos[i]
            .getValorDepositado().toString());
        FieldProviderFactory.setField(data[i], "rownum", "" + (i + 1));
               
        FieldProviderFactory.setField(data[i], "markSelectedId",
            ((ATEC_Depositos[i].getAlertStatus().equals("RDNC"))||(ATEC_Depositos[i].getAlertStatus().equals("PWNC"))) ? "" : ATEC_Depositos[i].getId());
      }
  ...
  public static BigDecimal getCurrentlyClearedAmt(String strAccountId) {
    ...
    hqlString.append("select sum(ft.valorDepositado) - sum(ft.valorPagado)");
    hqlString.append(" from atecdp_finacc_transaction_v as ft");
    hqlString.append(" left outer join ft.reconciliation as rec");
    hqlString.append(" where ft.financialAccount.id = '" + strAccountId + "'");
