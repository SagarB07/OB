...
import com.atrums.conciliacionec.dao.TransactionsDao;
import com.atrums.conciliacionec.process.FIN_TransactionProcess;
...
public class AddTransaction extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (vars.commandIn("DEFAULT")) {
       ...
       * vars.getRole(), strTabId)) || !(Utility.isElementInList( Utility.getContext(this, vars,
       * "#User_Client", strWindowId, accesslevel), vars.getClient()) && Utility.isElementInList(
       ...
       * Utility.messageBD(this, "NoWriteAccess", vars.getLanguage())); vars.setMessage(strTabId,
       * myError); printPageClosePopUp(response, vars); } else {
       */
      ...
      strNroDocumento = vars.getStringParameter("inpNroDocumento");
  ...
  private void saveAndCloseWindow(HttpServletResponse response, VariablesSecureApp vars,
    ...
    try {
      ...
      if (strTransactionType.equals("P")) { // Payment
        ...
        // traer cuenta financiera elegida en pantalla
        FIN_FinancialAccount account = OBDal.getInstance().get(FIN_FinancialAccount.class,
            strFinancialAccountId);

        for (FIN_Payment p : selectedPayments) {
          // cambiamos a cada pago el campo procesado a N
          ...
          // cambiamos para que se guarde con la cuenta financiera elegida
          // old: p.getAccount() - cuenta elegida en el payment
          // new: account - cuenta elegida en la pantalla para addTransaction
          FIN_FinaccTransaction finTrans = dao.getNewFinancialTransaction(p.getOrganization(),
              account, TransactionsDao.getTransactionMaxLineNo(account) + 10, p, description,
              ...
              p.getFinancialTransactionConvertRate(), p.getAmount(), strNroDocumento);

          // cambiamos el payment con processed Y
          // guardamos en bdd
          p.setProcessed(true);
          OBDal.getInstance().save(p);
        ...
        FIN_FinaccTransaction finTrans = dao.getNewFinancialTransaction(account.getOrganization(),
            ...
            FIN_Utility.getDate(strTransactionDate), null, null, null, null);
        OBError processTransactionError = processTransaction(vars, this, "P", finTrans);
        if (processTransactionError != null && "Error".equals(processTransactionError.getType())) {
          throw new OBException(processTransactionError.getMessage());
        }
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strOrgId,
    ...
    XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
        "com/atrums/conciliacionec/ad_actionbutton/AddTransaction").createXmlDocument();
  ...
  private void printGrid(HttpServletResponse response, VariablesSecureApp vars,
    ...
    XmlDocument xmlDocument = xmlEngine.readXmlTemplate(
        "com/atrums/conciliacionec/ad_actionbutton/AddTransactionGrid").createXmlDocument();
