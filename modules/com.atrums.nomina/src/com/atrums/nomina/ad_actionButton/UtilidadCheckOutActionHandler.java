...
public class UtilidadCheckOutActionHandler extends BaseActionHandler {
  ...
  protected JSONObject execute(Map<String, Object> parameters, String content) {
    ...
    try {
      ...
      final JSONArray utilidadesIds = jsonData.getJSONArray("utilidades");

      if (utilidadesIds.length() > 0) {
        // start with zero

        // check out all old stays of this guest
        for (int i = 0; i < utilidadesIds.length(); i++) {

          final String avanceId = utilidadesIds.getString(i);
          final noUtilidadLinea utilidadLine = OBDal.getInstance().get(noUtilidadLinea.class,
              avanceId);
          if (1 == 1) {
